package com.devpro.finalProjectJavaweb21.conf;
...
import com.devpro.finalProjectJavaweb21.services.UserDetailsServiceImpl;
...
public class SecureConf extends WebSecurityConfigurerAdapter {
	...
	protected void configure(final HttpSecurity http) throws Exception {
		http.csrf().disable().authorizeRequests() // bắt đầu cấu hình: tất cả các requests từ
				...
				.formLogin().loginPage("/sign_in").loginProcessingUrl("/login_processing_url")
				.successHandler(authenticationSuccessHandler())
//				.defaultSuccessUrl("/home", true) // login thành công thì
				// luôn trở về trang
				// người dùng
				.failureUrl("/sign_in?sign_in_error=true")

				.and()

				// cấu hình cho phần logout
				.logout().logoutUrl("/logout").logoutSuccessUrl("/sign_in").invalidateHttpSession(true)
